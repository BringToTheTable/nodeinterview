...
describe("Auth routes", () => {
    ...
    describe("GET /v1/user/profile ", () => {
        test("should return 400 Badrequest When user try to access profile with blacklisted token", async () => {
            await insertUsers([userOne]);
            let token;
            const loginCredentials = {
                email: userOne.email,
            ...
            await request(app).get("/v1/users/signout").set("token", token);

            res = await request(app).get("/v1/users/profile").set("token", token);
